...
const elNovoLink  = document.getElementById("novoLink")  as HTMLInputElement;
...
const inpBusca        = document.getElementById("filtroBusca")  as HTMLInputElement;
...
function normalizaGrupo(s: string) { return (s || "").trim(); }
...
function parseMoney(s: string): number {
  ...
  const n = Number(t); return isFinite(n) ? Number(n.toFixed(2)) : 0;
}
...
<li class="tr ${i.feito ? "done" : ""}" data-id="${i.id}">
  ...
  <div class="td">
    <input class="input slim" value="${formatMoney(i.valor||0)}" data-action="edit-valor" />
  </div>
  <div class="td">
    <div class="row gap">
      <input class="input flex" value="${esc(i.item)}" data-action="edit-item" />
      <input class="input flex" placeholder="link" value="${esc(i.link||"")}" data-action="edit-link" />
      ${linkHtml}
    </div>
  </div>
</li>`;
...
function render() {
  ...
  // grupo corrigido (case-insensitive; "Todos" não filtra)
  if (grupoFiltro && grupoFiltro.toLowerCase() !== "todos") {
    exibidos = exibidos.filter(i =>
      normalizaGrupo(i.grupo).toLowerCase() === grupoFiltro.toLowerCase()
    );
  }
  ...
  exibidos.sort((a,b) =>
    (Number(a.feito)-Number(b.feito)) || (b.prio-a.prio) || a.item.localeCompare(b.item,"pt-BR")
  );

  ulLista.innerHTML = exibidos.map(row).join("");
...
btnAdd.addEventListener("click", () => {
  ...
  const prio = clamp(Number(elNovaPrio.value || 5), 1, 10);
  ...
  const it: Item = {
    ...
    item: txt, prio, grupo, valor, link: link || undefined,
    feito:false, createdAt: Date.now()
  };
  items.push(it);
  elNovoItem.value = ""; elNovoValor.value=""; elNovoLink.value="";
  render();
...
btnCriarGrupo.addEventListener("click", () => {
  ...
  if (!grupos.map(s=>s.toLowerCase()).includes(g.toLowerCase())) {
    grupos.push(g); grupos.sort(cmpGrupo);
...
ulLista.addEventListener("input", (e) => {
  ...
  if (action==="edit-prio"){
    const v = clamp(Number((t as HTMLInputElement).value || 5),1,10);
    it.prio=v; persistUpdate(id,{prio:v}).catch(console.error);
  }else if(action==="edit-grupo"){
    const g = normalizaGrupo((t as HTMLSelectElement).value);
    it.grupo=g; persistUpdate(id,{grupo:g}).catch(console.error);
  }else if(action==="edit-valor"){
    const v = parseMoney((t as HTMLInputElement).value);
    it.valor=v; (t as HTMLInputElement).value = formatMoney(v);
...
onSnapshot(colItems, async snap => {
  ...
  snap.forEach(d => {
    ...
    const legacyName = v.item ?? v.nome ?? v.text ?? v.texto ?? "";
    const needsFix   = v.item === undefined && (v.nome || v.text || v.texto);

    arr.push({
      id: d.id,
      item: String(legacyName),
    ...
    if (needsFix) {
      fixes.push(updateDoc(doc(colItems, d.id), { item: String(legacyName) }).catch(()=>{}));
    }
  });

  items = arr;
  render();
  if (fixes.length) Promise.allSettled(fixes).catch(()=>{});
});
...
(async function start(){
  ...
  catch (e) { console.error("Falha na inicialização do Firebase. Verifique Auth anônima e .env.local.", e); }
  renderGrupos(); render();
})();
